...
From the Gamboa field season in 2021-2022, we collared 2 full groups; Galaxy and Trago, with GPS and audio collars. In Galaxy group, we discovered that they exhibited fission fusion behaviour. In this report, we will look further into these fission events to explore the properties of these sub-grouping patterns.

![](https://github.com/EmilyMayGrout/gif/raw/main/moveVis_satellite_coati060122.gif)
...
For each timestamp used in the analysis, **all** group members must have had a fix.
...
### 2: Proportion of individuals in each sub-group
We used the 50m radius as the cut-off distance to define the sub-groups. The data was split to when the group was divided into 2 and 3 sub-groups accordingly. This allowed us to look at the number of individuals in each sub-group, to understand how the sub-groups arrange themselves when the group has fissioned. 
...
```
When the group was split in 2 sub-groups, the majority of splits were either even with 5 individuals in one group and 6 in the other, or the split was from one individual outside of the group, whilst the rest of the group were together.
...
Again, when the group was split into 3 sub-groups, the group sizes were either 5, 5, 1, or 9, 1, 1. Therefore, we can show that when groups split, they either split evenly or the majority of the group is together with 1 or 2 individuals away from the group. The tables below give the counts of individuals in the different split compositions.
...
```
It is clear that coatis in Galaxy group spend more time with certain individuals, so it could be speculated that they have preferences for who they remain with when the group splits. We can also see that one of the sub-groups contains all adult females and the only juvenile of the group, whereas the other sub-group contains 2 adult females and 3 sub-adults. Gus, the adult male spends slightly more time with the mostly adult group, but very little time with Estrella. 
...
We did the same analysis for when all group members were together to determine whether there is within-group spatial clustering. To do this, we changed the epsilon neighbourhood value (radius) to 10m and 1m. So if the group is spread 30m, we would be able to identify which individuals are closer together. 

### Plot 5: Gus in the group with 10m radius
```{r , echo = FALSE, fig.align = "center", fig.height = 5, fig.width = 5}
R = 50
subgroup_data <- get_subgroup_data(xs, ys, R)
...
### Plot 6: Gus in the group with 3m radius
```{r , echo = FALSE, fig.align = "center", fig.height = 5, fig.width = 5}
...
all_tracked_idxs <- which(n_tracked==n_inds)
#find index for when there is only 1 subgroup
s1 <- which(subgroup_data$n_subgroups == 3)
...
within_group_data <- get_proximity_data(subset_x, subset_y, 1)
new_order <- c(1,11,4,10,2,3,6,7,8,9,5)
visualize_network_matrix(within_group_data$proximity_net, coati_ids[new_order,])

```
These matrices show that there may be some preferences within the full group. Quasar tends to be closer to Pluto, Luna, and Gus. Planeta tends to be closer to Orbita.
...
### Plot 7: Without Gus in the group with 10m radius
...
### Plot 8: Without Gus in the group with 3m radius
