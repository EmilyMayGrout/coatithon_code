#this code is making violin plot for subgroup sizes for both groups for each hour of the day
...
#gg <- 
  ggplot(n_subs, aes(panama_time_hms, n_groups, color = group))+
...
n_subs %>%
  mutate(time_of_day = cut(panama_time, breaks = c(-Inf, 7, 10, 14, 17, Inf), labels =  c("WakeUp", "Morning", "Midday", "Evening", "BeforeBed"))) -> n_subs

n_subs_gal %>%
  mutate(time_of_day = cut(panama_time, breaks = c(-Inf, 7, 10, 14, 17, Inf), labels = c("WakeUp", "Morning", "Midday", "Evening", "BeforeBed"))) -> n_subs_gal

n_subs_pres %>%
  mutate(time_of_day = cut(panama_time, breaks = c(-Inf, 7, 10, 14, 17, Inf), labels = c("WakeUp", "Morning", "Midday", "Evening", "BeforeBed"))) -> n_subs_pres

n_subs %>% 
  filter(!is.na(n_groups)) %>%
  ggplot(aes(n_groups, fill = group)) +
  geom_bar(position = "dodge") +
  facet_grid(. ~ time_of_day)

gg <- n_subs %>% 
  filter(!is.na(n_groups)) %>%
  ggplot(aes(time_of_day, fill = group)) +
  geom_bar(position = "dodge") +
  theme_classic()+
...
ggsave(filename = paste0(plot_dir, 'n_groups_time_of_day.png'), plot = gg, width = 6, height = 4, dpi = 300)


assocplot(table(n_subs_gal[,c("time_of_day", "n_groups")]), col = c("aquamarine3", "indianred1"))



# gam - to show the effect isn't strong 
library(mgcv)
gam1 <- gam(n_groups ~ panama_time_hms, data = n_subs_gal, family = "poisson")
