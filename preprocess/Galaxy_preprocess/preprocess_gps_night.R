...
#create times vector - only for the times that are in the NIGHT 00:00 (7pm) - 10:50 (5:50am)
#first make list of 10 min intervals then remove the times overnight
ts <- seq.POSIXt(from = firsttime, to = lasttime,  by = '10 mins')
...
save(list=c('xs','ys','ts'), file = paste0(outdir,'galaxy_xy_night_level0.RData'))
save(list=c('lats','lons','ts'), file = paste0(outdir,'galaxy_latlon_night_level0.RData'))  
