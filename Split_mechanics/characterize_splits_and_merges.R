...
if(use_manual_events){
  ...
  ff_data <- identify_splits_and_merges_new(R_inner = R_inner, R_outer = R_outer, xs, ys, ts, breaks = c(1, length(ts)+1), names = coati_ids$name_short, break_by_day = F, verbose = T)
  events <- ff_data$events_detected
}

if(group == 'galaxy'){
  save(events, file = paste0(groupdir, "gal_events.RData"))
} else if(group == 'presedente'){
  save(events, file = paste0(groupdir, "pres_events.RData"))  
}
